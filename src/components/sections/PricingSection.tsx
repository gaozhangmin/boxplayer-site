...
import { Check, Minus, ArrowRight, Loader2, CheckCircle } from "lucide-react";
...
const I18N = {
  en: {
    ...
    colFeature: "Feature", colFree: "Free", colPro: "Pro · $10/mo",
    features_EN: [
    ...
    cta: "Download the app and login to upgrade to Pro.",
    ctaBtn: "Get Started",
  },
  zh: {
    badge: "价格",
    ...
    colFeature: "功能", colFree: "免费版", colPro: "专业版 · $10/月",
    features_ZH: [
      { name: "网盘文件管理", free: true, pro: true },
      { name: "视频播放 & 音乐播放", free: true, pro: true },
      { name: "本地书籍阅读 (PDF/EPUB)", free: true, pro: true },
      { name: "多网盘同时连接", free: true, pro: true },
      { name: "AI 智能搜索", free: "5次/天", pro: "无限" },
      { name: "全网资源搜索", free: "5次/天", pro: "无限" },
      { name: "全网资源一键保存", free: false, pro: true },
      { name: "AI 文件整理 & 查重", free: false, pro: true },
      { name: "AI 阅读助手 (PDF/EPUB)", free: false, pro: true },
      { name: "语音朗读 (5000字/天)", free: false, pro: true },
      { name: "即时翻译 (5000字/天)", free: false, pro: true },
      { name: "TMDB + 豆瓣电影发现", free: false, pro: true },
      { name: "优先技术支持", free: false, pro: true },
    ],
    successTitle: "支付成功！",
    successDesc: "正在激活 App Pro…",
    cta: "下载 App 并登录后升级到专业版。",
    ctaBtn: "开始使用",
  },
};
...
export default function PricingSection({ lang = "zh" }: { lang?: "en" | "zh" }) {
  ...
  const t = I18N[lang];
  const features = lang === "en" ? t.features_EN : (t as any).features_ZH;

  return (
    <section id="pricing" aria-labelledby="pricing-heading" className="relative py-16 sm:py-24 md:py-32 bg-sky-50/30">
      {paymentSuccess && (
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-10">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-ink-100">
                <th className="py-4 px-4 sm:px-6 text-base font-semibold text-ink-900">{t.colFeature}</th>
                <th className="py-4 px-3 sm:px-4 text-center text-sm font-semibold text-ink-500">{t.colFree}</th>
                <th className="py-4 px-3 sm:px-4 text-center text-sm font-semibold text-skype-deep bg-amber-50/50">{t.colPro}</th>
              </tr>
            </thead>
            <tbody>
              {features.map((f: { name: string; free: boolean | string; pro: boolean | string }, i: number) => (
                <tr key={i} className="border-b border-ink-50">
                  <td className="py-3 px-4 sm:px-6 text-sm text-ink-700">{f.name}</td>
                  <td className="py-3 px-3 sm:px-4 text-center">
                    {f.free === true ? <Check className="w-4 h-4 text-emerald-500 mx-auto" strokeWidth={3} />
                    : f.free === false ? <Minus className="w-4 h-4 text-ink-300 mx-auto" strokeWidth={2} />
                    : <span className="text-xs text-ink-500">{f.free}</span>}
                  </td>
                  <td className="py-3 px-3 sm:px-4 text-center bg-amber-50/50">
                    {f.pro === true ? <Check className="w-4 h-4 text-skype-deep mx-auto" strokeWidth={3} />
                    : f.pro === false ? <Minus className="w-4 h-4 text-ink-300 mx-auto" strokeWidth={2} />
        ...
        <div className="text-center">
          <p className="text-ink-500 text-sm mb-4">{t.cta}</p>
          <Link href="/#download" className="inline-flex items-center gap-2 px-8 py-3.5 bg-skype-deep text-white rounded-xl font-semibold text-base hover:opacity-90 transition">{t.ctaBtn} <ArrowRight className="w-4 h-4" /></Link>
        </div>
      </div>
    </section>
  );
