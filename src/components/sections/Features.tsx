...
const I18N_FEATURES = {
  zh: { badge: "Why BoxPlayer", heading: <>Not just a player. <span className="italic text-skype-deep">A whole theatre.</span></>, desc: "iOS 4K 播放器、Apple TV 杜比视界播放器、Mac 原盘播放器、Windows / Linux 媒体服务器客户端 —— 一份订阅就能拿到 Plex 客户端、Jellyfin 客户端、Emby 客户端的能力,兼容 Windows / Linux / Docker / 群晖 / 威联通 / TrueNAS / unRAID 上自建的媒体服务器与 NFS 共享,是 Infuse、VidHub、Kodi、VLC、PotPlayer 的免费开源替代方案。", desc2: "播放器很多,但能把云盘、原盘、媒体服务器、字幕、跨设备进度一次做对的,只有 BoxPlayer。", features: FEATURES_ZH.concat(OLD_FEATURES) },
  en: { badge: "Why BoxPlayer", heading: <>Not just a player. <span className="italic text-skype-deep">A whole theatre.</span></>, desc: "A 4K Dolby Vision player, Plex client, Jellyfin client, and Emby client in one. Connect Windows/Linux/Docker/Synology/QNAP/TrueNAS media servers and NFS shares. The free open-source alternative to Infuse, VidHub, Kodi, and VLC.", desc2: "Many players exist. Only BoxPlayer gets cloud drives, BD remux, media servers, subtitles, and cross-device progress right in one package.", features: FEATURES_EN.concat(OLD_FEATURES_EN) },
};
